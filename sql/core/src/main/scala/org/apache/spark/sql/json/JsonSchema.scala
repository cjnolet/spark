...
/**
 * Provides functions for working with schemas that have been extracted
 * from JSON objects and arrays.
 */
object JsonSchema {

  /**
   * Allows a flattened schema representation to be aggregated using an Accumulator. This is useful
   * for inferring a schema as other processing is occurring.
   */
  implicit object FlattenedKeysValueTypesAccumulator extends AccumulatorParam[Set[(String, DataType)]] {
   ...
   * (in the format of Map[String, Any]). When the value of a key is a JSON object, we
   ...
   *
   * This will return a flattened representation of the overall schema in a
   * Set[(String, DataType)] so that it can be aggregated with other calls to this
   * function.  It is possible, however, that aggregating heterogeneous objects or
   * objects that may share similar key names could cause conflicts in the final Set.
   * This conflict is most likely to occur when a DataType representing a primitive value
   * is found representing a JSON object or a JSON array and aggregated together in the
   * same set. If this occurs, only one of the conflicted fields will be expanded
   * when createSchema() is called.
   */
  def flattenedKeysWithValueTypes(m: Map[String, Any]): Set[(String, DataType)] = {
    ...
    keyValuePairs.flatMap {
      case (key: String, struct: Map[_, _]) => {
        ...
        flattenedKeysWithValueTypes(struct.asInstanceOf[Map[String, Any]]).map {
        ...
        def buildKeyPathForInnerStructs(v: Any, t: DataType): Seq[(String, DataType)] = t match {
          case ArrayType(e: StructType, containsNull) => {
            ...
            v.asInstanceOf[Seq[Map[String, Any]]].flatMap(Option(_)).flatMap {
              element => flattenedKeysWithValueTypes(element)
            }.map {
              case (k, t) => (s"$key.$k", t)
            }
          }
  ...
  /**
   * When given a Set[(String, DataType)] of flattened schema elements, this function will
   * re-expand the schema into its nested form. Note that if the flattened schema
   * representation was based on a heterogeneous object type, it's possible there could be
   * conflicts in the datatypes (e.g. there are more than one DataTypes for a the
   * same field name). In these cases, only one will be included in the final StructType.
   * @param allKeys
   * @return
   */
  def createSchema(allKeys: Set[(String, DataType)]): StructType = {
  ...
  private[json] def asRow(json: Map[String,Any], schema: StructType): Row = {
